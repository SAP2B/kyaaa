...
fn main() {
    // 2. Heterogeneous registry with 2 Dogs and 2 Cats
    let shelter = book!(
        dog1 => Dog::new().name("Rex").age(3),
        dog2 => Dog::new().name("Thor").age(5),
        cat1 => Cat::new().name("Luna").age(2),
        cat2 => Cat::new().name("Mina").age(1)
    );
    ...
    // 4. Safe mutation on specific instances
